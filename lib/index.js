...
class StartdServer extends _react.default.Component {
  ...
  async componentDidMount() {
    ...
    if (this.state.inputMiddlewarePath) {
      ...
      const middlewarePath = _path.default.resolve(process.cwd(), // $FlowFixMe flow isn't picking up the check above
    ...
    const startd = new _startd.default(appPath);
    ...
    if (this.state.devMode) {
      ...
      this.addLog(`App successfully running production build. Your app is listening on port ${_chalk.default.magenta(this.state.port.toString())}`);
    }
  }
